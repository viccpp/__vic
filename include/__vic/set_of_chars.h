...
public:
    ...
    template<class Iter>
    set_of_chars(Iter begin, Iter end) { add(begin, end); }
    set_of_chars(const char *c_str) { add(c_str); }
    set_of_chars(std::initializer_list<char> set) { add(set); }
#else
    set_of_chars() { clear(); }
    template<class Iter>
...
#if __cpp_initializer_lists
    void add(std::initializer_list<char> set)
    {
